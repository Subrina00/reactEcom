  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        ...
        console.error('Eroor fetching product', error);
        ...
        <section>
          {categories.map((category, index) => (
            <label key={index} className='block mb-2'>
              ...
              checked={selectedCategory === category} className='mr-2 w-[16px] h-[16px]' />
        ...
        <div className='mb-5 mt-4'>
          ...
          <div>
            {keywords.map((keyword, index) => (
              <button key={index}
              onClick={() => handleKeywordClick(keyword)} className='block mb-2 px-4 w-full text-left border rounded hover:bg-gray-200'>{keyword.toUpperCase()}</button>
            ))}
          </div>
        </div>
